...
window.onload = function () {
  ...
  for (let i = 0; i < log_num; i++) {
    const itemName = "link" + i;
    links[i] = localStorage.getItem(itemName);
...
function SearchButtonClick() {
  ...
  for (let i = 1; i <= keyword_len; i++) {
    keyword = document.querySelector(
      `#input_not_keyword:nth-child(${i}) > input.form-control`
    ).value;
    console.log(keyword);
    if (keyword !== "") {
      search_link += encodeURI(`-"${keyword}"` + " ");
  ...
  for (let i = 1; i <= keyword_len; i++) {
    keyword = document.querySelector(
      `#input_or_keyword:nth-child(${i}) > input.form-control`
    ).value;
  ...
  search_link += ")";
  if (counter === 0) {
    search_link = search_link.substring(0, search_link.length - 2);
  }
  console.log(search_link);
  num += 1;
  ...
  if (keyword !== "") {
    search_link += `since%3A${keyword.substring(0, 4)}-${keyword.substring(
      4,
      2
    )}-${keyword.substring(6, 2)}%20`;
    arr[num] = true;
  }
  console.log(search_link);
  num += 1;
  ...
  if (keyword !== "") {
    search_link += `until%3A${keyword.substring(0, 4)}-${keyword.substring(
      4,
      2
    )}-${keyword.substring(6, 2)}%20`;
    arr[num] = true;
